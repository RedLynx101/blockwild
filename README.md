...
**Engineering case study:** [Building Blockwild with Agentic Autoresearch](BLOCKWILD_AGENTIC_AUTORESEARCH_CASE_STUDY.md) documents the human-directed multi-agent workflow, repository-scale feedback system, production audits, and telemetry-led optimization work behind the game.
...
Surface POIs generated by terrain workers now retain their semantic landmark, chest, resident, and encounter records when installed or restored from cache. Previously, the physical structure could exist while its map identity was silently discarded. The terrain cache advances once to a metadata-aware namespace so older voxel-only records regenerate safely while player edits remain preserved.
...
Double-left-clicking an inventory stack once again gathers every visible matching item up to the stack limit. Same-slot placement remains reliable: the release now waits for the browser's immediately following click before committing, so the second click can select collect-all, with a zero-delay fallback for overlays that replace a slot before that click arrives.

## v1.8.7 First Dawn Hotfix

The first main quest now measures completed survival days, so entering a new world on day one no longer completes “A Fire Through First Dawn”; day-one saves carrying the old false-ready state repair themselves. Inventory cursor stacks commit on a same-slot left-button release instead of remaining trapped in drag-paint mode.

Tree felling now removes the original standing mesh before its falling proxy begins moving on the host and every guest. Only sections containing removed tree voxels rebuild synchronously; neighboring seams and the larger lighting halo remain frame-budgeted to preserve the optimized cut response.
...
Wild Bonds turns the Bestiary into a creature-care, research, travel, and real-time combat system. The catalog now contains 215 production creatures, including 27 regular additions, six authored regional legendaries, and four stable-lineage summons. Every new species has fixed readable stats rather than hidden rolls, a level-and-bond move path, authored ecology and capture conditions, visible individual history, explicit sound coverage, and a purpose beyond collection. A universal 21-type resolver now covers player, creature, summon, construct, settlement, boss, projectile, spell, status, and mounted attacks through one host-authoritative pipeline.

The Bestiary uses composable multi-select facets and an overflow-safe rail instead of adding more category buttons. Research, specimens, capture history, Prime routes, care clues, mounts, forms, and summoned origins can grow as append-only chapters. Thirty-seven expansion rigs received a dedicated silhouette and detail pass with curved topology, layered materials, secondary motion, transparency, negative space, emissive accents, and concept-appropriate hovering or rotation; the same geometry drives gameplay, portraits, and the comparison renderer.

Guilds of Hearthroads adds seven faction-connected institutions, 21 principal characters, seven recruits, 56 authored campaign chapters, deterministic halls and rare lodges, six-rank progression, doctrine choices, separate standing, and exact semantic objective proof. Hall discovery and named invitations gate membership; chapter actions must occur at the marked site with the correct creature, actor, item, encounter, or route context, so shared verbs cannot cross-credit unrelated guilds. Sparse terrain-following regional roads retain their graph in world saves, create physical ferry landings, and use bounded persistent event anchors instead of scanning whole routes.

Twenty reusable spells now share Bestiary types and journal facets. Four unusual summons come from the Unwalked Meridian, Palimpsest Expanse, Hush Between Bells, and Sea Behind Mirrors; stable contracts prevent rerolls and duplication, while a timed Worldpin act can ground one free, persistent individual without silently taming it. Contextual loot now resolves by structure, room, biome/depth, faction, danger, history, and ownership, with frozen inventories, host-only rolls, private-container consequences, unique-item ledgers, and deterministic recovery paths.

The release preserves v1 and v2 Bestiary records, legacy cages and Capture Orbs, current worlds, generator-v15 terrain, host multiplayer authority, Dwarven mountain holds, and the optimized ecology/spatial-index contracts. Run `npm run audit:living-bestiary` for the 10,000-container, 50-seed road, 50-seed Dwarven, guild, legendary, and summon audit; `npm run showcase:living-bestiary:compare` rebuilds the production before/after sheets.
...
- **Adjustable performance policy.** Render distance defaults to 10 chunks and scales to 16, while simulation distance defaults to 8 and never exceeds the view. Adaptive counts and a fair wall-clock budget divide new terrain, voxel-light propagation, and mesh construction into resumable strips so exploration cannot schedule several whole chunks or sections in one frame. The occupied chunk has a bounded priority lane through generation, lighting, and the player's vertical mesh band; farther partial work pauses without being discarded and resumes after local terrain is drawable. Old section geometry remains visible until its replacement is complete, stale off-route work is discarded, redundant not-yet-built seam remeshes are suppressed, and optional CPU and memory reserve modes trade additional local resources for steadier streaming. The settings menu can enable a compact live FPS counter or download local diagnostics with separate generation, lighting, meshing, queue pressure, and player-chunk readiness measurements.
...
`ChunkWorld` owns deterministic generation, streamed chunk sections, geometry, terrain edits, and the derived voxel-light field. Each loaded voxel packs independent four-bit sky/red/green/blue channels; topology-aware increase/decrease propagation, smooth corner samples, and light-only section-buffer updates keep caves and placed lights correct without radius-wide remeshing. The complete contract and extension checklist live in [`docs/LIGHTING_SYSTEM.md`](docs/LIGHTING_SYSTEM.md). Generated terrain and lighting are reproducible from the seed, so saves store player-made edits rather than copies of generated blocks or light arrays.

`mob-models.ts` and `living-bestiary-models.ts` are the canonical builders for the 223 non-butterfly creatures, including six articulated dragon families, sentient residents, golems, Courser breeds, rabbit families, sea slugs, fish, core wildlife, authored adventure encounters, underground species, mythic creatures, and summons. The game engine instantiates those models directly, while the inspection script extracts their actual posed Three.js geometry for grounding checks and Bestiary portraits. Eight butterfly builders complete the current 231-entry field guide. `held-items.ts` reuses those builders and centralizes player-held geometry, while aquariums, conservatories, attuned Capture Orbs, Waygrid archives, racks, healers, eggs, mount cargo, and faction pet sales preserve the same exact creature records.
...
The focused release suites cover all 24 biomes, seven underground habitats, nine atmosphere recipes and atlas ownership, exact 28-POI/6-dungeon catalogs, connected variable dungeon tiles, wrought-door state families, reversible underground traversal, nine adventure-creature rigs, six legendary contracts, HRTF audio, bounded bird routing, attached rabbit ears, tree topology and full felling, waterlogged flora, renewable water and nonrenewing food liquids, Sugarcourt production, map/weather plans, exact apiary transfers, attuned-orb faint/recall rules, guild/road/loot semantics, universal creature combat and mounts, summon grounding, Waygrid capacity/search/overflow/area crafting, host-authoritative reconnect state, trade atomicity, connected variable-sized culture settlements, aquariums, shields, character profiles, Glimmerwood ecology, six golem rigs and their forging/defense contracts, six five-stage dragon families, Sea Dragon nests/charts/incubation, Gold and Silver celestial hatch rites, 20 spells, per-skill Ascendant traits, responsive interfaces, all 231 production-model portraits, and all 52 flora entries.
...
To render all 231 Bestiary entries as individual front-three-quarter portraits and a field-guide sheet:
