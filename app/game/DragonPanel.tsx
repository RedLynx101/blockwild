...
export function DragonPanel({ dragon, displayName, portrait, onClose, onCommand, onToggleShoulder, onHarvestScales, onOpenCargo }: DragonPanelProps) {
  ...
  const dragonLevel = Math.max(1, Math.min(1_000, Math.floor(ageDays * 2) + dragon.stage * 3));
      ...
      <div className="dragon-care-panel">
        ...
        <div className="dragon-care-grid">
          <aside>
            ...
            <div className="dragon-care-skills" aria-label={`Dragon level ${dragonLevel} skill attributes`}>
              <div><b>LEVEL</b><strong>{dragonLevel}</strong></div>
              <div><b>{dragon.type === "sea" ? "SWIM" : "FLIGHT"}</b><strong>{Math.min(1000, dragonLevel + (dragon.type === "sea" ? 90 : 45))}</strong></div>
              <div><b>POWER</b><strong>{Math.min(1000, dragonLevel + dragon.stage * 14)}</strong></div>
              <div><b>WARD</b><strong>{Math.min(1000, dragonLevel + armored * 25)}</strong></div>
            </div>
...
.dragon-care-fire{--dragon-glow:#ef6840;--dragon-soft:#6f3028}.dragon-care-ice{--dragon-glow:#78d9f3;--dragon-soft:#294d66}.dragon-care-steel{--dragon-glow:#b5c8cc;--dragon-soft:#46545c}.dragon-care-sea{--dragon-glow:#69eee5;--dragon-soft:#225d69}
...
.dragon-care-skills{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:14px}.dragon-care-skills>div{display:grid;gap:2px;padding:8px;border:1px solid #665248;background:#151719;text-align:center}.dragon-care-skills b{font-size:9px;letter-spacing:.14em;color:#aaa095}.dragon-care-skills strong{color:var(--dragon-glow);font-size:17px}
