...
.dragon-magic-wheelOverlay{position:fixed;z-index:120;inset:0;display:grid;place-items:center;background:radial-gradient(circle,#161b1ba8 0 14%,#090b0bd9 64%);backdrop-filter:blur(4px);animation:dragon-magic-fade .14s ease-out}.dragon-magic-wheel{position:relative;width:min(640px,92vmin);aspect-ratio:1;border-radius:50%;border:1px solid #8c8368;background:radial-gradient(circle,#242925 0 22%,#151816ed 23% 52%,#2c302c 52.5% 53%,transparent 53.5%);box-shadow:0 0 0 10px #0005,0 24px 90px #000;animation:dragon-magic-wheel-bloom .2s cubic-bezier(.2,.85,.2,1)}
...
.dragon-magic-wheelEmpty{display:grid;justify-items:center;gap:10px;text-align:center}.dragon-magic-wheelEmpty kbd{display:grid;width:42px;height:42px;place-items:center;border:1px solid #aa9c70;background:#111;color:#f4d98d;box-shadow:inset 0 -3px #000;font:700 15px inherit}.dragon-magic-wheelEmpty strong{font:700 clamp(20px,3vw,28px)/1.1 Georgia,serif}.dragon-magic-wheelEmpty p{max-width:360px;margin:0;color:#bbbdb2;font-size:12px;line-height:1.6}.dragon-magic-wheelEmpty small{color:#d8aa50;font-size:9px;letter-spacing:.12em;text-transform:uppercase}.dragon-magic-wheelDismiss{position:absolute;right:18px;top:18px;z-index:3;border:1px solid #65685e;background:#222622;color:#e9e2cf;padding:8px 10px;cursor:pointer}
.dragon-magic-wheel[data-empty="true"]{width:min(460px,calc(100vw - 32px));min-height:250px;aspect-ratio:auto;border-radius:2px;background:linear-gradient(145deg,#202420f2,#111411f5);box-shadow:0 20px 70px #000b,inset 0 0 0 3px #ffffff08;padding:54px 34px 32px}.dragon-magic-wheel[data-empty="true"]:before{inset:12px;border:1px solid #d8aa5030;border-radius:0;animation:none}.dragon-magic-wheel[data-empty="true"]:after{display:none}.dragon-magic-wheel[data-empty="true"] .dragon-magic-wheelDismiss{top:12px;right:12px;border:0;background:transparent;color:#a9aa9f;padding:8px}.dragon-magic-wheel[data-empty="true"] .dragon-magic-wheelDismiss:hover{color:#f4d98d}
...
export function SpellWheelPanel({ open, magic, onSelectSpell, onClose }: SpellWheelPanelProps) {
  ...
  return (
    <section className="dragon-magic-root dragon-magic-wheelOverlay" role="dialog" aria-modal="true" aria-label="Favorite spell wheel" onPointerDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <DragonMagicStyles />
      <div className="dragon-magic-wheel" data-empty={!slots.length}>
        <button type="button" className="dragon-magic-wheelDismiss" onClick={onClose} aria-label="Close spell wheel">{slots.length ? "Release Q" : "Close"}</button>
        ...
        }) : <div className="dragon-magic-wheelEmpty"><kbd>Q</kbd><strong>No favorite spells</strong><p>Open the Spell Journal to add learned spells here.</p><small>Release Q to return</small></div>}
        {slots.length ? <div className="dragon-magic-wheelCenter"><kbd>Q</kbd><strong>{selected?.name ?? "Spell Wheel"}</strong><small aria-label={`${slots.length} of ${MAX_FAVORITE_SPELLS} favorites`}>{`${slots.length} / ${MAX_FAVORITE_SPELLS} favorites · release to close`}</small></div> : null}
      </div>
    </section>
  );
}
