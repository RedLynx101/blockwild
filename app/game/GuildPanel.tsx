...
import { GUILDS, GUILD_NPCS, GUILD_QUESTS, promotionEligibility, questProgress, type GuildBookState, type GuildId } from "./guilds";
...
export function GuildPanel({ state, onClose, onJoin, onStartQuest, onResolveQuest, onPromote }: GuildPanelProps) {
  ...
  return <section className="menu-overlay guild-overlay" aria-labelledby="guild-panel-title">
    <div className="pixel-panel guild-folio">
      ...
      <header className="guild-folio-header">
        ...
        <dl><div><dt>Membership</dt><dd>{player.membership}</dd></div><div><dt>Rank</dt><dd>{currentRank}</dd></div><div><dt>Standing</dt><dd>{Math.round(player.standing)}</dd></div></dl>
      </header>
      <nav className="guild-tabs" aria-label="Guilds">
        {ids.map((id) => <button type="button" key={id} className={selected === id ? "active" : ""} aria-pressed={selected === id} onClick={() => setSelected(id)}><span>{GUILDS[id].name}</span><small>{state.guilds[id].membership === "unknown" ? "UNMET" : GUILDS[id].ranks.find((rank) => rank.id === state.guilds[id].rankId)?.name ?? "MEMBER"}</small></button>)}
      </nav>
      <div className="guild-folio-body">
        <main>
          <section className="guild-chapter-list" aria-labelledby="guild-campaign-title"><header><div><small>AUTHORED CAMPAIGN</small><h3 id="guild-campaign-title">Eight chapters</h3></div>{player.membership === "unknown" && <button type="button" className="guild-primary" onClick={() => onJoin(selected)}>Take the oath</button>}</header>
            {quests.map((quest) => {
              ...
              return <article key={quest.id} className={`${complete ? "complete" : ""} ${active ? "active" : ""}`}>
                ...
                <div><h4>{quest.name}</h4><p>{quest.summary}</p><small>{progress?.explanation ?? quest.solutionFamilies.join(" / ")}</small><details><summary>Consequences and recovery</summary><p>{quest.persistentChange}</p><p>{quest.recovery}</p></details></div>
                <div className="guild-chapter-state">{complete ? <b>RECORDED</b> : active && progress?.complete
                  ...
                  : active ? <b>IN FIELD</b> : <button type="button" disabled={player.membership === "unknown" || !unlocked} onClick={() => onStartQuest(quest.id)}>Accept</button>}</div>
              </article>;
            })}
          </section>
        </main>
        <aside>
          ...
          <section><small>PEOPLE, NOT LOADOUTS</small><h3>Hall company</h3>{people.map((person) => <article className="guild-person" key={person.id}><div><strong>{person.name}</strong><small>{person.role}{person.recruitable ? " · RECRUITABLE" : ""}</small></div><p>{person.philosophy}</p>{person.companion && <em>{person.companion}</em>}</article>)}</section>
