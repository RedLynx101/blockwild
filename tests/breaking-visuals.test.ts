...
import { BREAK_CRACK_STAGES, breakCrackStage, breakCrackStroke, toolEffectivenessForIds } from "../app/game/breaking-visuals.ts";
...
test("break crack stages advance monotonically and replace the default progress meter", () => {
  ...
  assert.equal(breakCrackStroke(0), "rgba(8, 7, 5, 0.52)");
  assert.equal(breakCrackStroke(7), "rgba(8, 7, 5, 0.94)");
});
