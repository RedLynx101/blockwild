...
test("rare appearance envelopes validate strictly", () => {
  ...
  const scope = { centerPlayerId: "player_rare_appearance_test", radius: 64, epoch: 1 };
  assert.equal(validatePayload("mob-snapshot", { tick: 2, scope, mobs: [entry] }), true);
  assert.equal(validatePayload("mob-snapshot", { tick: 2, scope, mobs: [{ ...entry, primeAnchorId: "copied:prime" }] }), false);
  assert.equal(validatePayload("mob-snapshot", { tick: 2, scope, mobs: [{ ...entry, appearance: { ...entry.appearance!, phenotype: { ...phenotype, markingMask: 99 } } }] }), false);
  assert.equal(validatePayload("mob-snapshot", { tick: 2, scope, mobs: [{ ...entry, specimenId: "x".repeat(161) }] }), false);
});
